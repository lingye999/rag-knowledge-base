...
def read_pdf(path: str) -> str:
    ...
    for page in doc:
        ...
        # 过滤出文本块（b[6]==0），按 y0 排序
        text_blocks = [(b[1], b[0], b[4].strip()) for b in blocks if b[6] == 0 and b[4].strip()]
        if not text_blocks:
            empty_pages += 1
            continue

        # 按 y 坐标分组（同一行内的块纵坐标相差 <5 视为同一行）
        text_blocks.sort(key=lambda b: (b[0], b[1]))  # y 优先，再 x
        lines = []
        current_y = text_blocks[0][0]
        current_line = []
        for y, x, t in text_blocks:
            if abs(y - current_y) < 5:
                # 同一行：按 x 顺序拼接
                current_line.append((x, t))
            else:
                # 换行：把上一行按 x 排序后输出
                current_line.sort(key=lambda p: p[0])
                lines.append("  ".join([p[1] for p in current_line]))
                current_y = y
                current_line = [(x, t)]
        # 最后一行
        if current_line:
